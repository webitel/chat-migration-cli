...
func (s *ConversationStore) GetGroupedConversationsByUsersAndFlow(ctx context.Context, lastSeenInitiatorID int, lastSeenFlowID int, limit int) ([]*old.GroupedConversation, error) {
	var (
		query = `
WITH conversations AS (SELECT conv.id,
     ...
     grouped_conversations AS (SELECT ARRAY_AGG(conv.id)                   conv_ids,
                                      initiator,
                                      flow_id,
                                      STRING_AGG(DISTINCT conv.title, ',') "title",
...
LEFT JOIN LATERAL (SELECT JSONB_AGG(users.user) internal_users
                            FROM (SELECT JSONB_BUILD_OBJECT('channel_id', array_agg(ch.id),
                                                            'user_id', ch.user_id,
...
`
	...
	rows, err := s.db.Pool().Query(ctx, query, lastSeenInitiatorID, lastSeenFlowID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
